...
static int mt9v113_set_antibanding(uint32_t antibanding_value)
{
	int rc = 0;
...
static int mt9v113_set_sharpness(uint32_t sharpness_value)
{
	int rc = 0;
...
static int mt9v113_set_saturation(uint32_t saturation_value)
{
	int rc = 0;
...
static int mt9v113_set_contrast(uint32_t contrast_value)
{
	int rc = 0;
...
static int mt9v113_set_brightness(uint32_t brightness_value)
{
	int rc = 0;
	if (op_mode == SENSOR_SNAPSHOT_MODE)
		return 0;
...
static int mt9v113_set_wb(uint32_t wb_value)
{
	int rc = 0, k = 0;
	unsigned short check_value;
...
static int mt9v113_set_iso(uint32_t iso_value)
{
	int rc = 0, k = 0;
	unsigned short check_value;
...
int32_t mt9v113_sensor_config(struct msm_sensor_ctrl_t *s_ctrl, void __user *argp)
{
	...
	case CFG_SET_BRIGHTNESS:
		...
		break;
	case CFG_SET_WB:
		rc = mt9v113_set_wb(cfg_data.cfg.wb_val);
		break;
...
#endif
	case CFG_SET_ISO:
		rc = mt9v113_set_iso(cfg_data.cfg.iso_type);
		break;
	case CFG_SET_FPS:
