	...
	Ilp0100_structSensorParams *YushanII_sensor)
{
	...
	pr_info("[CAM]%s",__func__);
        if (msm_sensor_get_csi_params(sensor, &csi_lane_params) < 0)
          {
            pr_err("[CAM]%s: unable to get csi params\n", __func__);
            return ;
          }
	op_pixel_clk = 
	...
	Ilp0100_structFrameFormat *output_format)
{
	...
	if(sensor->sensordata->hdr_mode){
		...
		output_format->Hoffset = 368;
		output_format->Voffset = 216;
	}
	else{
...
void YushanII_init_backcam(struct msm_sensor_ctrl_t *sensor,int res){
	...
	if(sensor->sensordata->hdr_mode){
		...
		Ilp0100_defineMode(output_format);
		pr_info(" [CAM]%s, Ilp0100_startTestMode TEST_NO_TEST_MODE", __func__);
		#ifdef HDR_COLOR_BAR
		Ilp0100_startTestMode(BYPASS_NO_BYPASS, TEST_COLORBAR);
		#endif
	}
...
static struct msm_cam_clk_info rawchip_clk_info[] = {
  // #ifdef CONFIG_RAWCHIP_MCLK cond here ..
	{"cam_clk", MSM_SENSOR_MCLK_24HZ},
};

int YushanII_power_up(struct msm_camera_rawchip_info *pdata)
{
	int rc = 0;
	CDBG("[CAM] %s\n", __func__);
	...
	if (rc < 0) {
        ...
        rc = msm_cam_clk_enable(pdata->dev, rawchip_clk_info,
			pdata->cam_clk, ARRAY_SIZE(rawchip_clk_info), 1);
        if (rc < 0) {
...
enable_reset_failed:
        rc = msm_cam_clk_enable(pdata->dev, rawchip_clk_info,
			pdata->cam_clk, ARRAY_SIZE(rawchip_clk_info), 0);
        if (rc < 0)
...
int YushanII_power_down(struct msm_camera_rawchip_info *pdata)
{
	int rc = 0;
	pr_info("%s\n", __func__);
...
static int YushanII_driver_probe(struct platform_device *pdev)
{
	...
	YushanIICtrl->pdata->dev = &pdev->dev;
